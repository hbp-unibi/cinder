...
/**
 * Generates a spike train according to the Poisson model of spike generation.
 * Spike times are generated by randomly selecting the inter-spike interval from
 * an exponential distribution.
 *
 * @param spike_train is the spike train to which the spikes should be added.
 * @param t_start is the start time.
 * @param t_end is the end time.
 * @param lambda is the number of events that on average is expected to occur in
 * the course of one second.
 * @param seed if -1 a random seed is chosen, otherwise the given seed is used.
 * @return the very vector that has been passed as "spike_train" parameter.
 */
static inline std::vector<Time> &poisson_spike_train(
    std::vector<Time> &spike_train, Time t_start, Time t_end, float lambda,
    int seed = -1)
{
	if (lambda > 0.0) {
		std::default_random_engine gen(seed == -1 ? std::random_device()()
		                                          : seed);
 ...
 * the course of one second.
 * @param seed if -1 a random seed is chosen, otherwise the given seed is used.
 * @return a new spike train vector containing the generated spike times. The
 * times stored in the vector are guaranteed to be sorted.
 */
static inline std::vector<Time> poisson_spike_train(Time t_start, Time t_end,
                                                    float lambda, int seed = -1)
{
	std::vector<Time> spike_train;
	poisson_spike_train(spike_train, t_start, t_end, lambda, seed);
	return spike_train;
}
