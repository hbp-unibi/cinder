...
struct CurAlphaTestRecorder {
	                           ...
	                           const std::vector<Time> &spikes)
	{
		...
		for (const Time &spike : spikes) {
			Time t2 = t - spike;
			if (t2 >= 0_s) {
				res += cur.v() / tau.sec() * t2.sec() *
				       std::exp(1.0 - t2.sec() / tau.sec());
			}
		}
		return res;
	}
...
TEST(cur_alpha, basic)
{
	auto current_source =
	    make_current_source(CurAlpha(10_nA, 10_ms, input_spikes1),
	                        CurAlpha(30_nA, 10_ms, input_spikes2),
	                        CurAlpha(-20_nA, 50_ms, input_spikes3));
	DormandPrinceIntegrator integrator;
	CurAlphaTestRecorder recorder;
